...
async def test_list_chats_paginates_with_page_token():
    graph = MagicMock()
    with_url = MagicMock()
    with_url.get = AsyncMock(
        return_value=_fake_collection([_fake_chat()], next_link=None)
    )
...
async def test_list_chat_messages_include_body():
    ...
    result = await teams_chats.list_chat_messages(
        graph=graph, chat_id="c1", include_body=True
    )
